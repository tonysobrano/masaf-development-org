...
export default async function HomePage() {
  ...
  if (sanityClient) {
    try {
      featuredItems = await sanityClient.fetch(featuredItemsQuery);
    } catch (error) {
  ...
  return (
    <>
      <HeroSection />
      <FeaturedCardsSection fetchedItems={featuredItems} />
      <WhoWeAreSection />
      <MissionSection />
      <WhyMasafSection />
      <ImpactSection />
...
function formatCategoryName(category: string): string {
  switch (category) {
    ...
    case "successStories": return "Success Story";
...
function FeaturedCardsSection({ fetchedItems }: { fetchedItems?: FeaturedItem[] }) {
  let items = [
    {
      _id: "card-1",
  ...
  if (fetchedItems && fetchedItems.length > 0) {
    items = fetchedItems.map(item => ({
      ...
      image: item.coverImage ? sanityImageUrl(item.coverImage, 600) : "/images/gallery/gallery-11.jpg",
      coverImage: item.coverImage,
      date: formatDate(item.date),
      category: formatCategoryName(item.category),
      href: getHref(item.category, item.slug),
    }));
  }
        ...
        <ul className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
          {items.map((item) => (
            <li key={item._id} className="h-full">
              ...
              >
                <div className="relative h-48 flex-shrink-0 overflow-hidden bg-masaf-tan/10">
                  <Image
                    src={item.image}
                    alt=""
                    fill
                  ...
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-masaf-ink/50 via-transparent to-transparent pointer-events-none" />
