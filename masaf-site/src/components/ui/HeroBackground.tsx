...
const HUB_COUNT = 6; // larger "community hub" nodes

function buildParticles(w: number, h: number): Particle[] {
  // More particles on bigger screens; minimum 60 so mobile is rich too
  ...
  return Array.from({ length: count }, (_, i) => {
    ...
    return {
      ...
      vy: isHub
        ...
        : -(Math.random() * 0.14 + 0.04), // upward drift – youth rising
      r: isHub ? Math.random() * 4 + 5 : Math.random() * 3 + 1.5,
      baseAlpha: isHub ? 0.92 : Math.random() * 0.50 + 0.42, // 0.42–0.92
...
export function HeroBackground() {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (let i = 0; i < particles.length; i++) {
        ...
        for (let j = i + 1; j < particles.length; j++) {
          ...
          if (dist2 < LINK_DIST2) {
            ...
            // Hub connections are brighter and thicker
      ...
      // Draw particles with two-layer glow
      for (const p of particles) {
  ...
  return (
    <>
      {/* CSS ambient orbs – instant render, no hydration wait */}
